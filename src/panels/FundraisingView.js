...
import {Button, FixedLayout, PanelHeaderBack, PanelHeaderButton, Progress, Separator, TabsItem} from "@vkontakte/vkui";
...
const FundraisingView = ({id, go}) => {
    ...
    return (
        <Panel id={id}>
            <PanelHeader separator={false} left={<PanelHeaderBack style={{color: "white"}} onClick={()=>{go("newsfeed")}}/>} transparent/>
            <img className="Fundraising_img" src={data.image} style={{width: "100%", height: 196, objectFit:"cover", marginTop: -56}}/>
            <Div className="Fundraising_titles">
                ...
                <div className="Fundraising_type">{data.type === "targeted" ? "Закончится через n дней" : "Помощь нужна каждый месяц"}</div>
            </Div>
            <Separator />
            <Div>
