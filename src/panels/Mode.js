...
import {Button, PanelHeaderBack, Placeholder, Banner} from "@vkontakte/vkui";
import {Icon28CalendarOutline, Icon28TargetOutline} from "@vkontakte/icons";

const Mode = ({id, go, setFundMode}) => (
    <Panel centered id={id}>
        <PanelHeader left={<PanelHeaderBack onClick={()=>{go('home')}}/>}>Тип сбора</PanelHeader>
        <div className="v-center">
            <Banner
                onClick={()=>{setFundMode("targeted");  go("fundraisingOpts")}}
                before={<Icon28TargetOutline style={{color: "var(--accent)"}}/>}
                asideMode="expand"
                header="Целевой сбор"
                subheader="Когда есть определённая цель"/>
            <Banner
                onClick={()=>{setFundMode("regular"); go("fundraisingOpts")}}
